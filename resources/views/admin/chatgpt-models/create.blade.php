...
@section('title', 'Добавить модель ChatGPT')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">Добавить модель ChatGPT</h4>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.chatgpt-models.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('model_id') is-invalid @enderror"
                       id="model_id" name="model_id" value="{{ old('model_id') }}"
                       placeholder="Например: gpt-4-turbo" required>
                @error('model_id')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror"
                          ...
                          placeholder="Краткое описание модели">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-check"></i> Создать модель
            </button>
        </form>
    </div>
</div>
